...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 car D:/DoD Internship/Neuron/Model/CaR.mod\n");
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
   double _lha , _lhb ;
  minf = 1.0 / ( 1.0 + exp ( ( _lv + 29.0 ) / - 9.6 ) ) ;
   _lha = 10e6 * ( _lv + 94.5 ) / ( exp ( ( _lv + 94.5 ) / 5.12 ) - 1.0 ) ;
   _lhb = 84.2 * exp ( _lv / 13.0 ) ;
   htau = ( 1.0 / ( _lha + _lhb ) + htauBase ) * 1000.0 ;
