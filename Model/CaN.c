...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 can D:/DoD Internship/Neuron/Model/CaN.mod\n");
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
   double _lma , _lmb ;
  _lma = 39800.0 * ( _lv + 17.19 ) / ( exp ( ( _lv + 17.19 ) / 15.22 ) - 1.0 ) ;
   _lmb = 384.2 * exp ( _lv / 23.82 ) ;
   mtau = 1000.0 / ( _lma + _lmb ) ;
   minf = 1.0 / ( 1.0 + exp ( ( _lv + 3.0 ) / - 8.0 ) ) ;
   hinf = 0.21 * 1.0 / ( 1.0 + exp ( ( _lv + 74.8 ) / 6.5 ) ) + 0.79 ;
     return 0; }
