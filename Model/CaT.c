...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 cat D:/DoD Internship/Neuron/Model/CaT.mod\n");
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
  ...
  _lma = 14552.0 * ( _lv + 84.5 ) / ( exp ( ( _lv + 84.5 ) / 7.12 ) - 1.0 ) ;
   _lmb = 4984.2 * exp ( _lv / 13.0 ) ;
   mtau = ( ( 1000.0 / ( _lma + _lmb ) ) + 0.0022 ) / 2.0 ;
   minf = 1.0 / ( 1.0 + exp ( ( _lv - 9.0 + 72.0 ) / - 8.0 ) ) ;
   _lha = 2652.0 * ( _lv + 94.5 ) / ( exp ( ( _lv + 94.5 ) / 5.12 ) - 1.0 ) ;
   _lhb = 684.2 * exp ( _lv / 13.0 ) ;
   htau = ( 1000.0 / ( _lha + _lhb ) + 0.1 ) / 2.0 ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv - 7.0 + 93.0 ) / 5.0 ) ) ;
     return 0; }
...
static double _nrn_current(double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt, double _v){double _current=0.;v=_v;{ {
   pca = pbar * m * m * m * h * ( 100.0 ) ;
   ica = pca * ghk ( _threadargscomma_ v , cai , cao ) ;
   }
 _current += ica;
