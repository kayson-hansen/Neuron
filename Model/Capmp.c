...
#define nrn_init _nrn_init__pmp
#define _nrn_initial _nrn_initial__pmp
#define nrn_cur _nrn_cur__pmp
#define _nrn_current _nrn_current__pmp
#define nrn_jacob _nrn_jacob__pmp
#define nrn_state _nrn_state__pmp
#define _net_receive _net_receive__pmp 
#define states states__pmp 
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "setdata_pmp", _hoc_setdata,
 0, 0
};
 /* declare global and static user variables */
#define shDepth shDepth_pmp
 double shDepth = 100;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "area");
 	...
 	ivoc_help("help ?1 pmp D:/DoD Internship/Neuron/Model/Capmp.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcai = - ica / ( ( 1.0 + kb ) * shDepth * FARADAY * 2.0 ) * ( 1e7 ) + ( caiInit - cai ) / taucl ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dcai = Dcai  / (1. - dt*( ( ( ( - 1.0 ) ) ) / taucl )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    cai = cai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / taucl)))*(- ( ( ( - ica ) / ( ( 1.0 + kb ) * shDepth * FARADAY * 2.0 ) )*( ( 1e7 ) ) + ( ( caiInit ) ) / taucl ) / ( ( ( ( - 1.0 ) ) ) / taucl ) - cai) ;
   }
  return 0;
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "Capmp.mod";
static const char* nmodl_file_text = 
  "COMMENT\n"
  "\n"
  "capmp.mod\n"
