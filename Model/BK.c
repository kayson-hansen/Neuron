...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 bk D:/DoD Internship/Neuron/Model/BK.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dbkact = ( bkinf - bkact ) / bktau ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 rates ( _threadargscomma_ v , cai ) ;
 Dbkact = Dbkact  / (1. - dt*( ( ( ( - 1.0 ) ) ) / bktau )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ v , cai ) ;
    bkact = bkact + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / bktau)))*(- ( ( ( bkinf ) ) / bktau ) / ( ( ( ( - 1.0 ) ) ) / bktau ) - bkact) ;
   }
  return 0;
}
 
static int  rates ( _threadargsprotocomma_ double _lv , double _lcai ) {
    bkinf = 1.0 / ( 1.0 + exp ( ( ( - 56.449 + 104.52 * exp ( - 0.22964 * _lcai * 1000.0 ) + 295.68 * exp ( - 2.1571 * _lcai * 1000.0 ) ) - _lv ) / ( 25.0 / 1.6 ) ) ) ;
   pretau = 1.0 / ( 10.0 * ( exp ( - ( _lv + 100.0 - ( 25.0 - 55.7 + 136.9 * exp ( - 0.28 * _lcai ) ) ) / 63.6 ) + exp ( - ( 150.0 - ( _lv + 100.0 - ( 25.0 - 55.7 + 136.9 * exp ( - 0.28 * _lcai ) ) ) ) / 63.6 ) ) - 5.2 ) ;
   if ( pretau < 0.2 ) {
     pretau = 0.2 ;
     }
   bktau = ( ( ( ( 13.7 + 234.0 * exp ( - 0.72 * _lcai ) ) - 1.0 ) * ( ( pretau - 0.2 ) / 0.8 ) ) + 1.0 ) ;
     return 0; }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(bkact) - _p;  _dlist1[0] = &(Dbkact) - _p;
_first = 0;
}
