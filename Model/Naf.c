...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "boltz_naf", _hoc_boltz,
 "rates_naf", _hoc_rates,
 0, 0
};
#define boltz boltz_naf
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 naf D:/DoD Internship/Neuron/Model/Naf.mod\n");
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
    taum = 0.2 ;
   alphah = 5.0754E-4 * exp ( - 6.3213e-2 * _lv ) ;
   betah = 9.7529 * exp ( 0.13442 * _lv ) ;
   tauh = 0.4 + 1.0 / ( alphah + betah ) ;
    minf = boltz ( _threadargscomma_ _lv , mvh , mvs ) ;
   hinf = boltz ( _threadargscomma_ _lv , hvh , hvs ) ;
    return 0; }
...
double boltz ( _threadargsprotocomma_ double _lv , double _lvh , double _lvs ) {
   double _lboltz;
 _lboltz = 1.0 / ( 1.0 + exp ( - ( _lv - _lvh ) / _lvs ) ) ;
