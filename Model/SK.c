...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 4, "cvodeieq");
 	...
 	ivoc_help("help ?1 sk D:/DoD Internship/Neuron/Model/SK.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   ...
   Dskact = ( skact_inf - skact ) / tau ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 rates ( _threadargscomma_ cai ) ;
 Dskact = Dskact  / (1. - dt*( ( ( ( - 1.0 ) ) ) / tau )) ;
  return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
   rates ( _threadargscomma_ cai ) ;
    skact = skact + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / tau)))*(- ( ( ( skact_inf ) ) / tau ) / ( ( ( ( - 1.0 ) ) ) / tau ) - skact) ;
   }
  return 0;
}
...
static int  rates ( _threadargsprotocomma_ double _lcai ) {
   theta = _lcai / kd ;
   skact_inf = pow ( theta , 5.2 ) / ( 1.0 + pow ( theta , 5.2 ) ) ;
    return 0; }
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(skact) - _p;  _dlist1[0] = &(Dskact) - _p;
_first = 0;
}
